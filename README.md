## перед запуском сервера в режиме 'development' необходимо выполнить data_base/create_db.py для создания тестовой базы
## для запуска в режиме разработки воспользуйтесь файлом run.py с указанием FLASK_ENV = 'development'
## для запуска в режиме продакт воспользуйтесь файлом run.py с указанием FLASK_ENV = 'production' и пути к базе данных в app/constants.py DATA_BASE_PATH_FOR_PROD
